 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Mr="RemoteStore";class EC{constructor(e,n,r,i,s){this.localStore=e,this.datastore=n,this.asyncQueue=r,this.remoteSyncer={},this.ha=[],this.Pa=new Map,this.Ta=new Set,this.Ia=[],this.Ea=s,this.Ea.No(o=>{r.enqueueAndForget(async()=>{$r(this)&&(z(Mr,"Restarting streams for network reachability change."),await async function(u){const h=Q(u);h.Ta.add(4),await Eo(h),h.da.set("Unknown"),h.Ta.delete(4),await Xl(h)}(this))})}),this.da=new vC(r,i)}}async function Xl(t){if($r(t))for(const e of t.Ia)await e(!0)}async function Eo(t){for(const e of t.Ia)await e(!1)}function EE(t,e){const n=Q(t);n.Pa.has(e.targetId)||(n.Pa.set(e.targetId,e),Fd(n)?bd(n):Hi(n).M_()&&Md(n,e))}function Ld(t,e){const n=Q(t),r=Hi(n);n.Pa.delete(e),r.M_()&&wE(n,e),n.Pa.size===0&&(r.M_()?r.N_():$r(n)&&n.da.set("Unknown"))}function Md(t,e){if(t.Aa.Ke(e.targetId),e.resumeToken.approximateByteSize()>0||e.snapshotVersion.compareTo(G.min())>0){const n=t.remoteSyncer.getRemoteKeysForTarget(e.targetId).size;e=e.withExpectedCount(n)}Hi(t).H_(e)}function wE(t,e){t.Aa.Ke(e),Hi(t).J_(e)}function bd(t){t.Aa=new pA({getRemoteKeysForTarget:e=>t.remoteSyncer.getRemoteKeysForTarget(e),Rt:e=>t.Pa.get(e)||null,Pt:()=>t.datastore.serializer.databaseId}),Hi(t).start(),t.da._a()}function Fd(t){return $r(t)&&!Hi(t).F_()&&t.Pa.size>0}function $r(t){return Q(t).Ta.size===0}function TE(t){t.Aa=void 0}async function wC(t){t.da.set("Online")}async function TC(t){t.Pa.forEach((e,n)=>{Md(t,e)})}async function IC(t,e){TE(t),Fd(t)?(t.da.ca(e),bd(t)):t.da.set("Unknown")}async function SC(t,e,n){if(t.da.set("Online"),e instanceof oE&&e.state===2&&e.cause)try{await async function(i,s){const o=s.cause;for(const l of s.targetIds)i.Pa.has(l)&&(await i.remoteSyncer.rejectListen(l,o),i.Pa.delete(l),i.Aa.removeTarget(l))}(t,e)}catch(r){z(Mr,"Failed to remove targets %s: %s ",e.targetIds.join(","),r),await pl(t,r)}else if(e instanceof Pa?t.Aa.Xe(e):e instanceof sE?t.Aa.ot(e):t.Aa.nt(e),!n.isEqual(G.min()))try{const r=await yE(t.localStore);n.compareTo(r)>=0&&await function(s,o){const l=s.Aa.It(o);return l.targetChanges.forEach((u,h)=>{if(u.resumeToken.approximateByteSize()>0){const f=s.Pa.get(h);f&&s.Pa.set(h,f.withResumeToken(u.resumeToken,o))}}),l.targetMismatches.forEach((u,h)=>{const f=s.Pa.get(u);if(!f)return;s.Pa.set(u,f.withResumeToken(ze.EMPTY_BYTE_STRING,f.snapshotVersion)),wE(s,u);const g=new zn(f.target,u,h,f.sequenceNumber);Md(s,g)}),s.remoteSyncer.applyRemoteEvent(l)}(t,n)}catch(r){z(Mr,"Failed to raise snapshot:",r),await pl(t,r)}}async function pl(t,e,n){if(!$i(e))throw e;t.Ta.add(1),await Eo(t),t.da.set("Offline"),n||(n=()=>yE(t.localStore)),t.asyncQueue.enqueueRetryable(async()=>{z(Mr,"Retrying IndexedDB access"),await n(),t.Ta.delete(1),await Xl(t)})}function IE(t,e){return e().catch(n=>pl(t,n,e))}async function Yl(t){const e=Q(t),n=ar(e);let r=e.ha.length>0?e.ha[e.ha.length-1].batchId:wd;for(;AC(e);)try{const i=await oC(e.localStore,r);if(i===null){e.ha.length===0&&n.N_();break}r=i.batchId,CC(e,i)}catch(i){await pl(e,i)}SE(e)&&AE(e)}function AC(t){return $r(t)&&t.ha.length<10}function CC(t,e){t.ha.push(e);const n=ar(t);n.M_()&&n.Y_&&n.Z_(e.mutations)}function SE(t){return $r(t)&&!ar(t).F_()&&t.ha.length>0}function AE(t){ar(t).start()}async function RC(t){ar(t).ta()}async function PC(t){const e=ar(t);for(const n of t.ha)e.Z_(n.mutations)}async function kC(t,e,n){const r=t.ha.shift(),i=Pd.from(r,e,n);await IE(t,()=>t.remoteSyncer.applySuccessfulWrite(i)),await Yl(t)}async function NC(t,e){e&&ar(t).Y_&&await async function(r,i){if(function(o){return dA(o)&&o!==b.ABORTED}(i.code)){const s=r.ha.shift();ar(r).O_(),await IE(r,()=>r.remoteSyncer.rejectFailedWrite(s.batchId,i)),await Yl(r)}}(t,e),SE(t)&&AE(t)}async function hg(t,e){const n=Q(t);n.asyncQueue.verifyOperationInProgress(),z(Mr,"RemoteStore received new credentials");const r=$r(n);n.Ta.add(3),await Eo(n),r&&n.da.set("Unknown"),await n.remoteSyncer.handleCredentialChange(e),n.Ta.delete(3),await Xl(n)}async function DC(t,e){const n=Q(t);e?(n.Ta.delete(2),await Xl(n)):e||(n.Ta.add(2),await Eo(n),n.da.set("Unknown"))}function Hi(t){return t.Ra||(t.Ra=function(n,r,i){const s=Q(n);return s.ra(),new mC(r,s.connection,s.authCredentials,s.appCheckCredentials,s.serializer,i)}(t.datastore,t.asyncQueue,{e_:wC.bind(null,t),n_:TC.bind(null,t),i_:IC.bind(null,t),j_:SC.bind(null,t)}),t.Ia.push(async e=>{e?(t.Ra.O_(),Fd(t)?bd(t):t.da.set("Unknown")):(await t.Ra.stop(),TE(t))})),t.Ra}function ar(t){return t.Va||(t.Va=function(n,r,i){const s=Q(n);return s.ra(),new gC(r,s.connection,s.authCredentials,s.appCheckCredentials,s.serializer,i)}(t.datastore,t.asyncQueue,{e_:()=>Promise.resolve(),n_:RC.bind(null,t),i_:NC.bind(null,t),X_:PC.bind(null,t),ea:kC.bind(null,t)}),t.Ia.push(async e=>{e?(t.Va.O_(),await Yl(t)):(await t.Va.stop(),t.ha.length>0&&(z(Mr,`Stopping write stream with ${t.ha.length} pending writes`),t.ha=[]))})),t.Va}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
